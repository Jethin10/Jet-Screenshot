                    ...
                    if (info != null && info.Result != null)
                    {
                        ...
                        else
                        {
                            ...
                            if (!task.StopRequested && info.Job != TaskJob.ShareURL && !string.IsNullOrEmpty(result))
                            {
                                ...
                                bool showCaptureOverlay = CanShowCaptureOverlay(task, info);

                                if (showCaptureOverlay)
                                {
                                    task.KeepImage = true;
                                    CaptureOverlayForm.ShowCapture(task);
                                }
                                else if (info.TaskSettings.GeneralSettings.ShowToastNotificationAfterTaskCompleted && !string.IsNullOrEmpty(result) &&
                                    (!info.TaskSettings.GeneralSettings.DisableNotificationsOnFullscreen || !CaptureHelpers.IsActiveWindowFullscreen()))
                                {
                                    task.KeepImage = true;
