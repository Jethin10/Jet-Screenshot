...
namespace ShareX
{
    public partial class TaskThumbnailPanel : UserControl
    {
        ...
        public delegate void TaskThumbnailDragCompletedEventHandler(TaskThumbnailPanel panel, DragDropEffects effect);
        public event TaskThumbnailDragCompletedEventHandler ThumbnailDragCompleted;

        /// <summary>
        /// Set this to a pre-rendered drag thumbnail. During drag, this image
        /// will be shown as the cursor instead of the default Windows file icon.
        /// </summary>
        public Bitmap DragCursorImage { get; set; }
        ...
        protected void OnThumbnailDragCompleted(DragDropEffects effect)
        {
            ThumbnailDragCompleted?.Invoke(this, effect);
        }
        ...
        private void PbThumbnail_MouseMove(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left && dragBoxFromMouseDown != Rectangle.Empty && !dragBoxFromMouseDown.Contains(e.X, e.Y))
            {
                if (Task.Info != null && !string.IsNullOrEmpty(Task.Info.FilePath) && File.Exists(Task.Info.FilePath))
                {
                    ...
                    // Create drag cursor from DragCursorImage if set
                    CreateDragCursorInternal();
                    pbThumbnail.GiveFeedback += PbThumbnail_GiveFeedback;
        ...
        private void PbThumbnail_GiveFeedback(object sender, GiveFeedbackEventArgs e)
        {
            if (dragCursor != null)
            {
        ...
        // P/Invoke for creating large drag cursors
        ...
        private void CreateDragCursorInternal()
        {
            DisposeDragCursorInternal();
            ...
            if (DragCursorImage != null)
            {
                ...
                try
                {
                    hBitmap = DragCursorImage.GetHbitmap(Color.FromArgb(0, 0, 0, 0));
                    using (Bitmap mask = new Bitmap(DragCursorImage.Width, DragCursorImage.Height))
                    {
                    ...
                    {
                        fIcon = false,
                        xHotspot = DragCursorImage.Width / 2,
                        yHotspot = DragCursorImage.Height / 2,
                        hbmMask = hMask,
                        hbmColor = hBitmap
                    };
        ...
        private void DisposeDragCursorInternal()
        {
            if (dragCursor != null)
            {
                dragCursor.Dispose();
