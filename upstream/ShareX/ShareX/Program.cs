...
namespace ShareX
{
    ...
    {
        public const string AppName = "JetSnap";
        ...
        public static string LogsFilePath
        {
            get
            {
                ...
                string fileName = string.Format("JetSnap-Log-{0:yyyy-MM}.txt", DateTime.Now);
        ...
        private static void Main(string[] args)
        {
            ...
            using (SingleInstanceManager singleInstanceManager = new SingleInstanceManager(MutexName, PipeName, !MultiInstance, args))
            {
                if (!singleInstanceManager.IsSingleInstance || singleInstanceManager.IsFirstInstance)
                {
                    ...
                    if (restartRequested)
                    {
                        DebugHelper.WriteLine("JetSnap restarting.");
        ...
        private static void Run()
        {
            ...
            DebugHelper.WriteLine("JetSnap starting.");
        ...
        public static void CloseSequence()
        {
            if (!closeSequenceStarted)
            {
                ...
                DebugHelper.WriteLine("JetSnap closing.");

                WatchFolderManager?.Dispose();
                SettingManager.HistoryClose();
                SettingManager.SaveAllSettings();

                DebugHelper.WriteLine("JetSnap closed.");
            }
        }
            ...
            if (!Sandbox)
            {
                ...
                if (!Directory.Exists(PersonalFolder))
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        MessageBox.Show(sb.ToString(), "JetSnap - " + Resources.Error, MessageBoxButtons.OK, MessageBoxIcon.Error);
                        CustomPersonalPath = "";
                    }
                }
            }
                ...
                if (!path.Equals(currentPath, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    catch (UnauthorizedAccessException e)
                    {
                        ...
                        MessageBox.Show(string.Format(Resources.Program_WritePersonalPathConfig_Cant_access_to_file, PersonalPathConfigFilePath),
                            "JetSnap", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                    catch (Exception e)
                    {
                        DebugHelper.WriteException(e);
        ...
        private static void ApplyTrayStartupDefaults()
        {
            ...
            try
            {
                StartupState startupState = StartupManager.State;

                if (startupState != StartupState.DisabledByPolicy)
                {
